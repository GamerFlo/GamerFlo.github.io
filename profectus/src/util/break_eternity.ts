...
export function letterFormat(num: DecimalSource, precision?: number): string {
    if (precision == null) precision = projInfo.defaultDecimalsShown;
    num = new Decimal(num);
    if (isNaN(num.sign) || isNaN(num.layer) || isNaN(num.mag)) {
        return "NaN";
    ...
    if (num.sign < 0) {
        return "-" + letterFormat(num.neg(), precision);
    }
    if (num.mag === Number.POSITIVE_INFINITY) {
        return "Infinity";
    }
    ...
    if (num.lt(1e6)) {
        return format(num.div(1000), 2) + "k";
    }
    if (num.lt(1e9)) {
        return format(num.div(1e6), 2) + "M";
    }
    if (num.lt(1e12)) {
        return format(num.div(1e9), 2) + "B";
    }
    if (num.lt(1e15)) {
        return format(num.div(1e12), 2) + "T";
    }

    const letters = "abcdefghijklmnopqrstuvwxyz";
    const tier = Math.floor(num.log(1000).add(22).toNumber()); // Calculate tier directly using logarithm, start with aa at 1e15
    const value = Decimal.div(num, Decimal.pow(1000, tier-22)); // Adjust value based on tier

    let letter = "";
    let tempTier = tier;
    while (tempTier > 0) {
        letter = letters[(tempTier - 1) % 26] + letter;
        tempTier = Math.floor((tempTier - 1) / 26);
    }

    return format(value, precision) + letter;
}
