...
import { letterFormat } from "util/break_eternity";
...
export function displayResource(resource: Resource, overrideAmount?: DecimalSource, letter?: boolean): string {
    const amount = overrideAmount ?? resource.value;
    if (Decimal.eq(resource.precision, 0)) {
        return format(resource.small ? amount : Decimal.floor(amount), Decimal.lte(amount, 1e15) ? 0 : 2);
    }
    return format(amount, resource.precision);
}
